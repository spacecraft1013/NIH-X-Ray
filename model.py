...
class NIHXRayModel(nn.Module):
    def __init__(self, model_config: Config, input_size: tuple[int, int, int, int], num_classes: int = 14, classification_hidden_features: int = 2048):
        super().__init__()
    ...
    def build_model(self) -> None:
        ...
        for i in range(self.num_blocks):
            layers.append(self.conv_block(self.num_filters,
                          self.num_filters, self.kernel_size, self.stride))
    ...
    def output_block(self, in_channels) -> nn.Module:
        return nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(in_channels*(self.input_size[2]//(self.num_blocks*self.down_factor))*(
                self.input_size[3]//(self.num_blocks*self.down_factor)), self.classification_hidden_features),
